...
def get_dhcp_plugin_status():
    ...
    try:
        ...
        # Check if DHCP clients are configured
...
def add_dhcp_client():
    ...
    try:
        ...
        print(f"DHCP client added successfully")
...
def remove_dhcp_client(sw_if_index):
    ...
    try:
        ...
        # Create minimal client config for deletion
        client_config = {
            'sw_if_index': int(sw_if_index),
            'hostname': '',
            'id': b'',
        ...
        print(f"DHCP client removed successfully")
...
def add_dhcp_proxy():
    ...
    try:
        ...
        print(f"Adding DHCP proxy: rx_vrf={rx_vrf_id}, server={dhcp_server}, src={dhcp_src_address}")
        ...
        return jsonify({
            'success': True,
            'message': 'DHCP proxy added successfully'
        })
...
def remove_dhcp_proxy():
    ...
    try:
        ...
        print(f"DHCP proxy removed successfully")
        
        return jsonify({
            'success': True,
            'message': 'DHCP proxy removed successfully'
        })
    
    except Exception as e:
        error_trace = traceback.format_exc()
        print(f"Error in remove_dhcp_proxy: {e}\n{error_trace}")
        return jsonify({'error': str(e), 'trace': error_trace}), 500


# ============================================================================
# DHCP VSS (Virtual Subnet Selection) Configuration
# ============================================================================

@dhcp_bp.route('/api/dhcp/vss', methods=['POST'])
def set_dhcp_vss():
    """Set DHCP VSS (Virtual Subnet Selection)"""
    try:
        v = get_vpp_connection()
        if not v:
            return jsonify({'error': 'Not connected to VPP'}), 500
        ...
        vss_type = data.get('vss_type', 255)  # 255 = default
        ...
        print(f"DHCP VSS set successfully")
        
        return jsonify({
            'success': True,
            'message': 'DHCP VSS set successfully'
        })
...
def unset_dhcp_vss():
    """Unset DHCP VSS (Virtual Subnet Selection)"""
    try:
        v = get_vpp_connection()
        if not v:
            return jsonify({'error': 'Not connected to VPP'}), 500
        ...
        return jsonify({
            'success': True,
            'message': 'DHCP VSS unset successfully'
        })
    
    except Exception as e:
        error_trace = traceback.format_exc()
        print(f"Error in unset_dhcp_vss: {e}\n{error_trace}")
        return jsonify({'error': str(e), 'trace': error_trace}), 500
...
def enable_dhcpv6():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'DHCPv6 clients enabled'
        })
...
def disable_dhcpv6():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'DHCPv6 clients disabled'
        })
    
    except Exception as e:
        error_trace = traceback.format_exc()
        print(f"Error in disable_dhcpv6: {e}\n{error_trace}")
        return jsonify({'error': str(e), 'trace': error_trace}), 500


@dhcp_bp.route('/api/dhcp/v6/duid', methods=['POST'])
def set_dhcpv6_duid():
    """Set DHCPv6 DUID-LL (DHCP Unique Identifier - Link Layer)"""
    try:
        v = get_vpp_connection()
        if not v:
            return jsonify({'error': 'Not connected to VPP'}), 500
        ...
        # Convert duid_ll to bytes if it's a hex string
        ...
        return jsonify({
            'success': True,
            'message': 'DHCPv6 DUID-LL set successfully'
        })
    
    except Exception as e:
        error_trace = traceback.format_exc()
        print(f"Error in set_dhcpv6_duid: {e}\n{error_trace}")
        return jsonify({'error': str(e), 'trace': error_trace}), 500
...
def enable_dhcp_detect(sw_if_index):
    """Enable DHCP client detection on an interface"""
    try:
        v = get_vpp_connection()
        if not v:
            return jsonify({'error': 'Not connected to VPP'}), 500
        ...
        return jsonify({
            'success': True,
            'message': 'DHCP detection enabled',
            'sw_if_index': sw_if_index
        })
    
    except Exception as e:
        error_trace = traceback.format_exc()
        print(f"Error in enable_dhcp_detect: {e}\n{error_trace}")
        return jsonify({'error': str(e), 'trace': error_trace}), 500


@dhcp_bp.route('/api/dhcp/detect/<int:sw_if_index>', methods=['DELETE'])
def disable_dhcp_detect(sw_if_index):
    """Disable DHCP client detection on an interface"""
    try:
        v = get_vpp_connection()
        if not v:
            return jsonify({'error': 'Not connected to VPP'}), 500
        ...
        return jsonify({
            'success': True,
            'message': 'DHCP detection disabled',
            'sw_if_index': sw_if_index
        })
